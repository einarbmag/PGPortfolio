...
class Trader:
    def __init__(self, waiting_period, config, total_steps, net_dir, agent=None, initial_USDT=1.0, agent_type="nn"):
        """
        ...
        # the total assets is calculated with USDT
        self._total_capital = initial_USDT
        ...
        if self.__class__.__name__=="BackTest":
            # self._initialize_logging_data_frame(initial_USDT)
    ...
    def _initialize_logging_data_frame(self, initial_USDT):
        logging_dict = {'Total Asset (USDT)': initial_USDT, 'USDT': 1}
    ...
    def _log_trading_info(self, time, omega):
        ...
        if self._steps > 0:
            logging_dict = {'Total Asset (USDT)': self._total_capital, 'USDT': omega[0, 0]}
    ...
    def __trade_body(self):
        ...
        logging.info('total assets are %3f USDT' % self._total_capital)
        logging.debug("="*30)
