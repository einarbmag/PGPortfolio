...
def calculate_pv_after_commission(w1, w0, commission_rate):
    """
    @:param w1: target portfolio vector, first element is usdt
    @:param w0: rebalanced last period portfolio vector, first element is usdt
...
def asset_vector_to_dict(coin_list, vector, with_USDT=True):
    vector = np.squeeze(vector)
    dict_coin = {}
    if with_USDT:
        dict_coin['USDT'] = vector[0]
